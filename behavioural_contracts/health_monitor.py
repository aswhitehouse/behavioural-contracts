...
class HealthMonitor:
    def __init__(self, max_strikes: int = 3, strike_window_seconds: int = 3600):
        ...
        self.strike_window_seconds = strike_window_seconds
    ...
    def add_strike(self, reason: str = None):
        ...
        if self.strikes >= self.max_strikes:
            ...
            logger.warning(f"Agent marked as unhealthy after {self.strikes} strikes. Last unusual behavior: {reason}")
        else:
            logger.warning(f"Strike recorded: {reason}. Total strikes: {self.strikes}, Status: {self.status}")
