...
def behavioural_contract(contract_spec: Dict, contract_instance: Optional[BehaviouralContract] = None):
    # Use provided contract instance or create a new one
    contract = contract_instance or BehaviouralContract(contract_spec)
    logger.info(f"Behavioural contract decorator initialized with {'provided' if contract_instance else 'new'} instance")
    
    def decorator(fn: Callable):
        @functools.wraps(fn)
        def wrapper(*args, **kwargs):
            logger.info(f"Behavioural contract wrapper called for function: {fn.__name__}")
            ...
            for attempt in range(max_retries + 1):
                ...
                try:
                    ...
                    if contract.response_validator.validate(raw_response):
                        ...
                        context = {
                            'memory': kwargs.get('simulated_memories', []),
                        ...
                        if contract.is_suspicious_behavior(raw_response, context):
                            ...
                            # ⚠️ New behavior: Don't fallback — let downstream agent evaluate
