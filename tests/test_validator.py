...
from behavioural_contracts.validator import ResponseValidator, try_parse_json

dummy_policy: Dict[str, Any] = {
    "pii": False,
...
def test_required_fields_validation():
    ...
    dummy_policy = {"compliance_tags": ["EU-AI-ACT"]}
    dummy_behavioural_flags = {"temperature_control": {"range": [0.2, 0.6]}}
    dummy_response_contract = {"max_response_time_ms": 1000}

    # Valid response
    assert validator.validate(
        {"decision": "BUY", "confidence": "high", "compliance_tags": ["EU-AI-ACT"]},
        dummy_policy,
        dummy_behavioural_flags,
        dummy_response_contract,
    ) == (True, "")
...
def test_json_parsing():
    ...
    # Invalid JSON string
    invalid_str = (
        '{"recommendation": "BUY", "confidence": "high"'  # Missing closing brace
    )
